...
wspsa_selection = [0; 0; 0; 0]; %0:spsa, 1:wspsa(only wh), 2:wspsa(wh before cut,wc after cut), 3:wspsa(wh before cut,wc+wh after cut)
...
perturb_step_init = [8; 0.1; 0.2; 0.2]; %initial perturbation step size
perturb_step_min = [2; 0.05; 0.05; 0.05]; %minimum perturbation step size
perturb_reduce_speed = [1.5; 1.5; 1.5; 1.5]; %the speed to reduce the step size if there's no improvement
...
%                   SPSA      Parameters            %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
upper = [10^3; 20; 20; 1]; %the upper bound for the parameters during the calibration in ratio
...
all_a = [1*10^(-5);1*10^(-6);5*10^(-5) * spddsy_a;0] .* (A+1)^0.602;
c = 4;
percentage = 0.2;
