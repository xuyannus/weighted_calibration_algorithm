...
perfect_wmatrix=zeros(paramsPerDay, no_sen);
...
for k= nstart:nstart+n-1
    ...
    for xx = 1:grad_reps 
      ...
      %only change
      perfect_weightmatrix = nonweight(wmatrix_all, secondorder_all, theta,1);
      parfor ii = 1:paramsPerDay
          
          thisdiff = perfect_weightmatrix(ii, :) * ydiff;
   ...
   % aa = mean(tmp_fn_grad_reps(:,1:1));
    ...
    % parfor ii = 1:paramsPerDay
    %    init_sum(ii,1) = perfect_wmatrix(ii, :) * init;
    % end
